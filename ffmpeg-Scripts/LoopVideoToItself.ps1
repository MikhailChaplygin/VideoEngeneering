...
foreach ($filename in $filenames)
{
    ...
    $bitrate = &($ffmpeg_path+"ffprobe.exe") -v error -show_entries stream=bit_rate -of csv=p=0 $filename

 #   Write-Output $bitrate

    if ($duration -gt 0)
    { 
        &($ffmpeg_path+"ffmpeg.exe") -loglevel info -i "$filename" -filter_complex `
            "[0]split[body][pre];
